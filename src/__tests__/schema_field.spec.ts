...
describe('SchemaField, should', () => {
    ...
    it('correctly handle schema error event', async () => {
        ...
        expect(schemaField.getSchemaError()).toMatchObject({validations: [], errorType: 'test'});
    });
    ...
    it('correctly setField', async () => {
        ...
        window.addEventListener('message', (event) => {
            if(event.data.type === Message.SET_VALUE) {
                ...
                iframe.contentWindow?.postMessage({ type: Message.RESULT, messageId: event.data.messageId, result: {success: true, value: 'new-string-value'}}, '*');
            }
        })
