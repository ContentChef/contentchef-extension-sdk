...
describe('api, should', () => {
  ...
  it('correctly receive and send messages for content api', async () => {
    ...
    window.addEventListener('message', event => {
      ...
      if (event.data.type === Message.CREATE_CONTENT) {
        ...
        iframe.contentWindow?.postMessage(
          {
            messageId: event.data.messageId,
            result: { id: 'test', name: 'test-new-content' },
            type: Message.RESULT,
          },
          '*'
        );
      ...
      if (event.data.type === Message.UPDATE_CONTENT) {
        ...
        iframe.contentWindow?.postMessage(
          {
            messageId: event.data.messageId,
            result: { id: 'test', name: 'test-update-content' },
            type: Message.RESULT,
          },
          '*'
        );
    ...
    const contents = await api.content.listContent({
      definition: 'test-definition',
    });
    expect(contents).toBeDefined();
    expect(contents.length).toBeGreaterThan(0);

    const newContent = await api.content.createContent({
      name: 'test-new-content',
    });
    expect(newContent).toBeDefined();
    expect(newContent.name).toBe('test-new-content');

    const updatedContent = await api.content.updateContent({
  ...
  it('correctly receive and send messages for repository api', async () => {
    ...
    window.addEventListener('message', event => {
      ...
      if (event.data.type === Message.CREATE_REPOSITORY) {
        ...
        iframe.contentWindow?.postMessage(
          {
            messageId: event.data.messageId,
            result: { id: 'test', name: 'test-new-repository' },
            type: Message.RESULT,
          },
          '*'
        );
      ...
      if (event.data.type === Message.UPDATE_REPOSITORY) {
        ...
        iframe.contentWindow?.postMessage(
          {
            messageId: event.data.messageId,
            result: { id: 'test', name: 'test-update-repository' },
            type: Message.RESULT,
          },
          '*'
        );
    ...
    const repositories = await api.repository.listRepository({
      name: 'test-repository',
    });
    expect(repositories).toBeDefined();
    expect(repositories.length).toBeGreaterThan(0);

    const newRepository = await api.repository.createRepository({
      name: 'test-new-repository',
    });
    expect(newRepository).toBeDefined();
    expect(newRepository.name).toBe('test-new-repository');

    const updatedRepository = await api.repository.updateRepository({
  ...
  it('correctly receive and send messages for definition api', async () => {
    ...
    window.addEventListener('message', event => {
      ...
      if (event.data.type === Message.CREATE_DEFINITION) {
        ...
        iframe.contentWindow?.postMessage(
          {
            messageId: event.data.messageId,
            result: { id: 'test', name: 'test-new-definition' },
            type: Message.RESULT,
          },
          '*'
        );
      ...
      if (event.data.type === Message.UPDATE_DEFINITION) {
        ...
        iframe.contentWindow?.postMessage(
          {
            messageId: event.data.messageId,
            result: { id: 'test', name: 'test-update-definition' },
            type: Message.RESULT,
          },
          '*'
        );
    ...
    const definitions = await api.definition.listDefinition({
      name: 'test-definition',
    });
    expect(definitions).toBeDefined();
    expect(definitions.length).toBeGreaterThan(0);

    const newDefinition = await api.definition.createDefinition({
      name: 'test-new-definition',
    });
    expect(newDefinition).toBeDefined();
    expect(newDefinition.name).toBe('test-new-definition');

    const updatedDefinition = await api.definition.updateDefinition({
      name: 'test-update-definition',
    });
  ...
  it('correctly receive and send messages for publishing-channel api', async () => {
    ...
    window.addEventListener('message', event => {
      ...
      if (event.data.type === Message.CREATE_PUBLISHING_CHANNEL) {
        ...
        iframe.contentWindow?.postMessage(
          {
            messageId: event.data.messageId,
            result: { id: 'test', name: 'test-new-publishing-channel' },
            type: Message.RESULT,
          },
          '*'
        );
      ...
      if (event.data.type === Message.UPDATE_PUBLISHING_CHANNEL) {
        ...
        iframe.contentWindow?.postMessage(
          {
            messageId: event.data.messageId,
            result: { id: 'test', name: 'test-update-publishing-channel' },
            type: Message.RESULT,
          },
          '*'
        );
    ...
    const pubCh = await api.publishingChannel.getPublishingChannel({
      id: 'test',
    });
    expect(pubCh).toBeDefined();
    expect(pubCh.id).toBe('test');

    const pubChs = await api.publishingChannel.listPublishingChannel({
      name: 'test-publishing-channel',
    });
    expect(pubChs).toBeDefined();
    expect(pubChs.length).toBeGreaterThan(0);

    const newPubCh = await api.publishingChannel.createPublishingChannel({
      name: 'test-new-publishing-channel',
    });
    expect(newPubCh).toBeDefined();
    expect(newPubCh.name).toBe('test-new-publishing-channel');

    const updatedPubCh = await api.publishingChannel.updatePublishingChannel({
      name: 'test-update-publishing-channel',
    });
